...
from sklearn_quantile import (ExtraTreesQuantileRegressor, RandomForestMaximumRegressor, RandomForestQuantileRegressor,
                              SampleExtraTreesQuantileRegressor, SampleRandomForestQuantileRegressor)
...
def test_common_attributes(model):
    ...
    assert np.all(
        [
            np.may_share_memory(m.y_weights_, m.estimators_[i].y_weights_)
            for i in range(m.n_estimators)
        ]
    )
    assert np.all(
        [
            np.may_share_memory(m.y_train_leaves_, m.estimators_[i].y_train_leaves_)
            for i in range(m.n_estimators)
        ]
    )
...
@pytest.mark.parametrize(
    "model,base_model,passes",
    [
        (RandomForestQuantileRegressor, RandomForestRegressor, True),
...
def test_value_array_against_RF(model, base_model, passes):
    ...
    assert passes == np.all(
        [
            np.array_equal(
                m.estimators_[i].tree_.value, base_m.estimators_[i].tree_.value
            )
            for i in range(m.n_estimators)
        ]
    )
...
def test_maximum_regressor(n_jobs):
    ...
    rf = RandomForestMaximumRegressor(
        n_estimators=1, max_depth=2, bootstrap=False, n_jobs=n_jobs
    )
    rf.fit(x, y)
...
def test_maximum_rf_negative_values():
    # Generating training data with negative targets
    r = np.random.RandomState(0)
    X_train = r.rand(100, 5)
    X_test = r.rand(10, 5)

    y_train = -np.abs(np.random.randn(100) + 1)  # All Y values are negative

    rf = RandomForestMaximumRegressor()
    rf.fit(X_train, y_train)

    assert np.all(rf.predict(X_test) < 0)
