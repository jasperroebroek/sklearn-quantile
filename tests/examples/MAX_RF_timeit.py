...
qrf_default = QRF(n_estimators=100, random_state=0, n_jobs=-1, method='default', quantiles=1)
qrf_sample = QRF(n_estimators=100, random_state=0, n_jobs=-1, method='sample', quantiles=1)
max_rf = MAX_RF(n_estimators=100, random_state=0, n_jobs=-1)
...
for mul in [1, 10, 100, 1000]:
    ...
    print("maximum")
    start = time()
    max_rf.fit(c_X_train, c_y_train)
    print(f"- fit : \t{time() - start}")
    start = time()
    max_rf.predict(c_X_train)
    print(f"- predict : \t{time() - start}")
