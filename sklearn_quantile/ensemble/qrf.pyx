...
class RandomForestQuantileRegressor(_ForestQuantileRegressor):
    """
    A forest regressor providing quantile estimates.
    ...
    method : {'default', 'sample'}, optional
        Fitting and prediction methods. The ``default`` option uses the implementation
        of the original paper, which is slower but more accurate. The ``sample`` option
        uses the implementation of the quantregForest (R) package. This is a lot faster,
        but approximates the quantiles.
...
class ExtraTreesQuantileRegressor(_ForestQuantileRegressor):
    """
    An extra trees regressor providing quantile estimates.
